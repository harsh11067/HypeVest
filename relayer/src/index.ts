...
const creatorChannels = new Map<string, {
  channelId: string;
 ...
 */
async function fetchChannelAnalytics(channelId: string) {
  try {
 ...
 */
async function updateCreatorRevenue(creatorAddress: string, channelId: string) {
  try {
 ...
 */
async function recordRevenueOnChain(trancheId: string, revenueReport: any) {
  try {
    const message = JSON.stringify(revenueReport);
    const reportHash = ethers.keccak256(ethers.toUtf8Bytes(message));
